...
from .rotated_anchor_head_aqe import RotatedAnchorHeadAQE
import torch
import math
@ROTATED_HEADS.register_module()
class RotatedRetinaHeadAQE(RotatedAnchorHeadAQE):
    ...
    def __init__(self,
        ...
        super(RotatedRetinaHeadAQE, self).__init__(
            num_classes,
            in_channels,
    ...
    def forward_single(self, x):
        ...
        angle_pred = self.retina_angle(reg_feat)
        bbox_preds = torch.cat([bbox_pred, angle_pred], dim=1)
    ...
    def filter_bboxes(self, cls_scores, bbox_preds):
        ...
        for lvl in range(num_levels):
            ...
            bbox_pred = bbox_pred.reshape(num_imgs, -1, self.num_anchors, 6)
    ...
    def refine_bboxes(self, cls_scores, bbox_preds):
        ...
        for lvl in range(num_levels):
            bbox_pred = bbox_preds[lvl]
            bbox_pred = bbox_pred.permute(0, 2, 3, 1)
            bbox_pred = bbox_pred.reshape(num_imgs, -1, 6)
            anchors = mlvl_anchors[lvl]

            for img_id in range(num_imgs):
                bbox_pred_i = bbox_pred[img_id]
                bbox_pred_i = bbox_pred_i[:, :-1]
                bbox_pred_i[:, -1] = torch.sigmoid(bbox_pred_i[:, -1])*math.pi-math.pi/2
                decode_bbox_i = self.bbox_coder.decode_aqe(anchors, bbox_pred_i)
