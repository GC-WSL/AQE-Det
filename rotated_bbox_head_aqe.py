...
class RotatedBBoxHeadAQE(BaseModule):
    ...
    def __init__(self,
                 ...
                 loss_angle=dict(
                     type='SmoothL1Loss', beta=1.0, loss_weight=1.0),
                 init_cfg=None):
        super(RotatedBBoxHeadAQE, self).__init__(init_cfg)
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        if num_pos > 0:
            ...
            if not self.reg_decoded_bbox:
                pos_bbox_targets, w, h = self.bbox_coder.encode_aqe(
                    pos_bboxes, pos_gt_bboxes)
                #print(pos_bbox_targets[:,-1].max().item(), pos_bbox_targets[:,-1].min().item())
            else:
            ...
            #bbox_weights[:num_pos, 4] = 2.0*torch.clamp(2.0*(torch.exp(-3.38608021*((torch.min(w,h)/torch.max(w,h))**2))-0.04446743),0.5,1.0)
        if num_neg > 0:
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                if self.reg_decoded_bbox:
                    # When the regression loss (e.g. `IouLoss`,loss
                ...
                losses['loss_bbox'] = self.loss_bbox(
                    ...
                    bbox_weights[pos_inds.type(torch.bool), :-1],
                    avg_factor=bbox_targets.size(0),
                    reduction_override=reduction_override)
                
                losses['loss_angle'] = self.loss_angle(pos_bbox_pred[:,-2], pos_bbox_pred[:,-1], bbox_targets[pos_inds.type(torch.bool), -1], bbox_weights[pos_inds.type(torch.bool), -1], avg_factor=bbox_targets.size(0))
            else:
    ...
    def get_bboxes(self,
        ...
        # e.g. Grid R-CNN.
        if bbox_pred is not None:
            bboxes = self.bbox_coder.decode_aqe(
                rois[..., 1:], bbox_pred[:,:-1], max_shape=img_shape)
        else:
            bboxes = rois[:, 1:].clone()
        ...
        '''
        ###############AQ-NMS##############
        sigma = torch.sigmoid(bbox_pred[:,-1])
        weight = 0.1
        top_ind = (scores>=cfg.score_thr).reshape(-1,16) #####2 16
        aq_score = (1-sigma)
        scores = weight*aq_score.reshape(-1, 1)+scores*(1-weight)
        scores = torch.mul(scores, top_ind.float())
        ###############AQ-NMS##############
        '''
        if cfg is None:
    ...
    def regress_by_class(self, rois, label, bbox_pred, img_meta):
        ...
        assert bbox_pred.size(1) == 6

        if rois.size(1) == 4:
            new_rois = self.bbox_coder.decode_aqe(
                rois, bbox_pred[:,:-1], max_shape=img_meta['img_shape'])
        else:
            bboxes = self.bbox_coder.decode_aqe(
                rois[:, 1:], bbox_pred[:,:-1], max_shape=img_meta['img_shape'])
