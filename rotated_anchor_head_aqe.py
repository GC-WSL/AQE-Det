...
class RotatedAnchorHeadAQE(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 loss_angle=dict(type='aqe_loss', loss_weight=0.05),
                 train_cfg=None,
                 test_cfg=None,
                 init_cfg=dict(type='Normal', layer='Conv2d', std=0.01)):
        super(RotatedAnchorHeadAQE, self).__init__(init_cfg)
    ...
    def _get_targets_single(self,
        ...
        if len(pos_inds) > 0:
            if not self.reg_decoded_bbox:
                pos_bbox_targets = self.bbox_coder.encode_aqe(
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        sigma = bbox_pred[:, -1]
        xywh_pred = bbox_pred[:, :-1]#######
        # if self.reg_decoded_bbox:
        #anchors = anchors.reshape(-1, 5)
        #bbox_decoded = self.bbox_coder.decode(anchors, bbox_pred[:, :-1])
        angle_target = bbox_targets[:, -1]
        #print(angle_target.max().item(), (torch.sigmoid(xywh_pred[:,-1])*math.pi-math.pi/2).max().item(), angle_target.min().item(), (torch.sigmoid(xywh_pred[:,-1])*math.pi-math.pi/2).min().item())
        #bbox_targets[:, -1] = bbox_decoded[:, -1]
        loss_bbox = self.loss_bbox(
            xywh_pred[:,:-1],
            bbox_targets[:,:-1],
            bbox_weights[:,:-1],
            avg_factor=num_total_samples)
        loss_angle = self.loss_angle(xywh_pred[:,-1], sigma, angle_target, bbox_weights[:,-1], avg_factor=num_total_samples)
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, anchors in zip(cls_score_list,
            ...
            weight = 0.1
            top_ind = (scores>=cfg.score_thr).reshape(-1,1)
            aq_score = (1-sigma)
